...
export const HeroSection: React.FC<HeroSectionProps> = ({ onPlayClick }) => {
  return (
    <>
      {/* DESKTOP HERO LAYOUT (>= md): Reverted to original centered position (top: 57%) */}
      <div
        className="hidden md:flex absolute z-20 select-none flex-col items-center"
        style={{
          top: "57%",
          left: "50%",
      ...
      >
        <img
          src="/text.png"
          alt="HIDDEN GAMBIT"
          className="w-[380px] md:w-[420px] h-auto object-contain drop-shadow-[0_12px_28px_rgba(0,0,0,0.95)]"
        />

        {/* Sub-tagline */}
        <p className="mt-1.5 font-tech text-xs tracking-[0.25em] font-semibold text-neutral-400 uppercase drop-shadow-md">
          EVERY MOVE{" "}
        ...
        {onPlayClick && (
          ...
          >
            ...
            <span className="font-tech font-extrabold text-base tracking-[0.25em] uppercase text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)]">
              CREATE GAME
            </span>
          </button>
        )}
      ...
      {/* MOBILE HERO LAYOUT (< md): Top Logo + Bottom Tagline & Button for mobile_ui.png */}
      <div className="flex md:hidden flex-col">
        ...
        >
          <img
            src="/text.png"
            alt="HIDDEN GAMBIT"
            className="w-[280px] sm:w-[320px] h-auto object-contain drop-shadow-[0_12px_28px_rgba(0,0,0,0.95)]"
          />
        </div>

        {/* Bottom Tagline & Create Game Button */}
        <div
        ...
        >
          <p className="font-tech text-[10px] sm:text-xs tracking-[0.25em] font-semibold text-neutral-400 uppercase drop-shadow-md text-center">
            EVERY MOVE{" "}
            <span className="text-red-600 font-bold drop-shadow-[0_0_8px_rgba(239,68,68,0.8)]">HIDES</span>{" "}
            A{" "}
            <span className="text-red-600 font-bold drop-shadow-[0_0_8px_rgba(239,68,68,0.8)]">SECRET.</span>
          ...
          {onPlayClick && (
            <button
              onClick={onPlayClick}
              className="group relative mt-3 px-8 py-3.5 bg-gradient-to-r from-red-950 via-red-800 to-red-950 hover:from-red-900 hover:via-red-600 hover:to-red-900 border-2 border-red-500/80 rounded clip-chamfer-btn shadow-[0_0_30px_rgba(239,68,68,0.6)] flex items-center space-x-3 transform active:scale-95 transition-all duration-200 cursor-pointer pointer-events-auto overflow-hidden"
            >
              <div className="w-7 h-7 rounded-full bg-red-600/40 border border-red-400/60 flex items-center justify-center shrink-0">
                <Swords className="w-4 h-4 text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.9)]" />
              </div>
              <span className="font-tech font-extrabold text-sm tracking-[0.2em] uppercase text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)]">
                CREATE GAME
              </span>
            </button>
          )}
